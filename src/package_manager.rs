...
fn read_package_manager_field(
  root: &Path,
) -> Result<(PackageManager, Option<String>), DetectError> {
  ...
  if let Some(pm_str) = pj.packageManager {
    // formato: "<name>@<version>", ex: "pnpm@8.15.4", "yarn@3.5.1", "npm@9.9.0",
    // "bun@1.1.8"
...
mod tests {
  ...
  fn test_package_manager_install_commands() {
    ...
    assert_eq!(
      PackageManager::YarnClassic.install_command(),
      vec!["yarn", "add"]
    );
    assert_eq!(
    ...
    assert_eq!(
      PackageManager::Unknown.install_command(),
      vec!["npm", "install"]
    );
  }
  ...
  fn test_package_manager_install_dev_commands() {
    assert_eq!(
      PackageManager::Npm.install_dev_command(),
      vec!["npm", "install", "--save-dev"]
    );
    assert_eq!(
      PackageManager::YarnClassic.install_dev_command(),
      vec!["yarn", "add", "--dev"]
    );
    assert_eq!(
      PackageManager::YarnBerry.install_dev_command(),
      vec!["yarn", "add", "--dev"]
    );
    assert_eq!(
      PackageManager::Pnpm.install_dev_command(),
      vec!["pnpm", "add", "--save-dev"]
    );
    assert_eq!(
      PackageManager::Bun.install_dev_command(),
      vec!["bun", "add", "--dev"]
    );
    assert_eq!(
      PackageManager::Unknown.install_dev_command(),
      vec!["npm", "install", "--save-dev"]
    );
  }
...
impl PackageManager {
  ...
  pub fn install_dev_command(&self) -> Vec<String> {
    match self {
      ...
      PackageManager::YarnClassic => {
        vec!["yarn".to_string(), "add".to_string(), "--dev".to_string()]
      }
